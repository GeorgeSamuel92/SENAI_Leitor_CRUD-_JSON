...
async function insert(cliente, endereco, telefone) {
  ...
  try {
    ...
    const res = await connection.query(
      "INSERT INTO cliente (nome, dt_nasc, cpf, estado_civil, email, ) VALUES (?, ?, ?, ?)",
      [cliente.nome, cliente.dt_nasc, cliente.cpf, cliente.estado_civil]
    );
    ...
    telefone.forEach(async (tel) => {
      await connection.query(
        ...
        [res[0].insertId, tel.tipo, tel.numero]
      );
    });
    ...
    //   await connection.query(
    //     "INSERT INTO endereco (id_cliente, logradouro, numero, bairro, complemento, cep, cidade, uf) VALUES (?,?,?,?,?,?,?,?)",
    //     [
    //       res[0].insertId,
    //       endereco.logradouro,
    //       endereco.numero,
